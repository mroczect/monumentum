...
use crate::core::schema::column::{CheckConstraint, ColumnDef, ComparisonOp, DataType, ForeignKey};
...
impl Decode for TableSchema {
    fn decode(cursor: &mut Cursor<&[u8]>) -> Result<Self, DbError> {
        ...
        if col_count > crate::constants::MAX_COLUMNS {
...
impl Decode for Row {
    fn decode(cursor: &mut Cursor<&[u8]>) -> Result<Self, DbError> {
        ...
        if value_count > crate::constants::MAX_COLUMNS {
            return Err(DbError::corruption(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
...
impl Decode for Table {
    fn decode(cursor: &mut Cursor<&[u8]>) -> Result<Self, DbError> {
        ...
        if row_count > crate::constants::MAX_ROWS_PER_TABLE {
            return Err(DbError::corruption(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "too many rows",
            )));
...
impl Decode for Catalog {
    fn decode(cursor: &mut Cursor<&[u8]>) -> Result<Self, DbError> {
        ...
        if table_count > crate::constants::MAX_TABLES {
            return Err(DbError::corruption(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                "too many tables",
            )));
