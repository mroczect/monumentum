...
pub(crate) const TAG_FORMULA: u8 = 6;
...
impl Encode for crate::core::value::Value {
    fn encode(&self, buf: &mut Vec<u8>) -> Result<(), DbError> {
        match self {
            ...
            Self::Formula(s) => {
                TAG_FORMULA.encode(buf)?;
                s.as_bytes().encode(buf)?;
            }
        }
        Ok(())
    }
...
impl Decode for Vec<crate::core::value::Value> {
    fn decode(cursor: &mut Cursor<&[u8]>) -> Result<Self, DbError> {
...
impl Decode for crate::core::value::Value {
    fn decode(cursor: &mut Cursor<&[u8]>) -> Result<Self, DbError> {
...
pub fn encode_catalog(catalog: &crate::core::catalog::Catalog) -> Result<Vec<u8>, DbError> {
    let mut buf = Vec::new();
    catalog.encode(&mut buf)?;
    Ok(buf)
}

pub fn decode_catalog(data: &[u8]) -> Result<crate::core::catalog::Catalog, DbError> {
    let mut cursor = Cursor::new(data);
    crate::core::catalog::Catalog::decode(&mut cursor)
}
