...
pub struct JsonbExtractFunction;

impl ScalarFunction for JsonbExtractFunction {
    ...
    fn call(&self, args: &[Value]) -> Result<Value, DbError> {
        if args.len() < 2 {
            return Err(DbError::invalid_operation(
                "jsonb_extract requires at least 2 arguments",
            ));
        }
        ...
        if args.len() == 2 {
            ...
            for arg in &args[1..] {
                ...
                arr.push(r.unwrap_or(JsonValue::Null));
            }
