use core::f64::consts::{E, FRAC_PI_4, PI};
...
fn fv(x: f64) -> Result<Value, DbError> {
    Value::try_from(x)
}
...
fn test_degrees_radians() -> Result<(), DbError> {
    ...
    assert_float(&deg.call(&[fv(PI)?])?, 180.0)?;
    assert_float(&rad.call(&[fv(180.0)?])?, PI)?;
    Ok(())
}
...
fn test_atan2() -> Result<(), DbError> {
    ...
    if let Value::Float(v) = result {
        assert!((v.as_f64() - FRAC_PI_4).abs() < 1e-12);
    } else {
        return Err(DbError::type_mismatch("expected float"));
    }
    assert_eq!(f.call(&[fv(1.0)?])?, Value::Null);
